...
/**
 * 文言は実装事実に合わせる（公開プライバシーポリシーとも整合させる）。
 * - 残り予算＝予算 − 購入済み合計（候補は差し引かない。`utils/budget-core.ts`）
...
const FAQ: FaqItem[] = [
  {
    q: '候補と購入済みの違いは？',
    a: '「候補」は買うか迷っているもの、「購入済み」は買ったものです。残り予算は購入済みの合計だけを差し引いて表示します（候補は差し引きません）。',
  },
  {
    q: 'レートはどう決まりますか？',
  ...
  {
    q: '外部と通信する機能はありますか？',
    a: 'Proの購入・復元時はApp Storeと、課金管理サービス（RevenueCat）が購入情報を処理します。翻訳ページの音声入力はAppleの音声認識を使い、端末内で処理できない場合はAppleのサーバーで処理されることがあります。詳しくはプライバシーポリシーをご覧ください。',
  },
];
