...
【2026-07-22決定・2026-07-30現行実体監査で本ファイルへ反映】カメラ撮影・保存画像/スクリーンショット・テキスト入力の3方式による多言語OCR・翻訳・原文/日本語訳表示・金額抽出・円換算・商品/旅行記録としての保存を、後回し機能ではなく正式な中核構想として決定済み。詳細・判断順序の正本はVault `AI-Workflow-System/07_project-kits/tabirate-camera/旅レートカメラ_多言語OCR・翻訳・テキスト入力_中核構想設計書_v1.md`。

- **実装状況**：構想決定のみ。コード実装は未着手（現行の`expo-text-extractor`呼び出しは`recognitionLanguages`等の言語指定手段を持たず、OSデフォルト認識に依存）
- **方式の優先順位**：オンデバイスOCR・オンデバイス翻訳・端末内保存が第一候補。クラウドOCR・外部翻訳APIは既定路線ではなく、「オンデバイスが製品品質に届かなかった場合にのみ検討する未決定の代替案」
- **次アクション（着手時期は人間判断待ち）**：①iOSオンデバイス多言語OCRの仮実装 → ②オンデバイス翻訳の仮実装 → ③英/日/韓/中(簡体・繁体)/タイ語での実機精度検証 → ④品質判定 → （不足時のみ）⑤外部方式の比較検討
