...
- **`FREE_LIMITS.saves` の値を変えない**（現行値は `10`。2026-08-28のFree/Pro設計再整理で30→10へ変更済み。数値の正は `src/config/limits.ts`）。scopeは**1つの旅行につき10件**（旅行ごと。端末全体ではない。2026-09-11 Human確定）で、数える対象（`getHistoryCountForTrip`）も変えない。
